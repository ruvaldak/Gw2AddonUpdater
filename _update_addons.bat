...
powershell -Command "$response = Invoke-WebRequest https://api.github.com/repos/megai2/d912pxy/releases/latest -UseBasicParsing; $json = $response.Content | ConvertFrom-Json; $url = $json.assets[0].browser_download_Url; Invoke-WebRequest $url -OutFile d912pxy.zip; Expand-Archive d912pxy.zip -DestinationPath ./"
del /f d912pxy.zip
cd ..
